...
def softmax(x):
    ...
    #prevent overflow by removing max before exp
    e = np.exp(x - np.sum(x, axis=1, keepdims=True))
...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    dictt = {}
    dictt["W1"] = np.random.normal(0,1, size=(input_size,num_hidden))
    dictt["W2"] = np.random.normal(0,1, size=(num_hidden,num_output))
    dictt["b1"] = np.zeros(shape=(num_hidden))
    dictt["b2"] = np.zeros(shape=(num_output))
    return dictt
...
def forward_prop(data, labels, params):
    ...
    #loss 
    n = len(data)
    loss = -1/n * np.sum(np.sum(labels * np.log(a2), axis=1))

    tup = (a1,a2,loss) #a1 = input, a2 = output
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    da1 = np.multiply(np.dot(dz2,W2.T),(a1*(1-a1)))
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    dz2 = (a2 - labels)
    dW2 = np.dot(a1.T,dz2) + reg *(2*W2) * len(data)
    db2 = np.sum(dz2, axis=1, keepdims = True)
    
    da1 = np.multiply(np.dot(dz2,W2.T),(a1*(1-a1)))
    dW1 = np.dot(data.T,da1) + reg *(2*W1) * len(data)
    db1 = np.sum(da1, axis=1, keepdims = True)
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for i in range(0,50):
        dictt = backward_prop_func(train_data[start:end],train_labels[start:end],params,forward_prop_func)
        ...
        params["W1"] += -learning_rate * (dW1/batch_size)
        params["b1"] += -learning_rate * (db1/batch_size)
        params["W2"] += -learning_rate * (dW2/batch_size)
        params["b2"] += -learning_rate * (db2/batch_size)
